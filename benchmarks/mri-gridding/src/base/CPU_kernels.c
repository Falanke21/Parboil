          ...
          for(dy2=Dy2, ny=NyL; ny<=NyH; ++ny, ++dy2)
          {
            ...
            if(dy2dz2<cutoff2)
            {
              int len = NxH - NxL + 1;
                ...
                // int i = 0;
                // __m256 vec_NxL = _mm256_set1_ps(NxL);
                // __m256 vec_dy2dz2 = _mm256_set1_ps(dy2dz2);
                // __m256 vec_dx2 = _mm256_set1_ps(*dx2);
                // for (; i < upper_bound; i += stride) {
                //   __m256 vec_nx = vec_NxL + (float)i;
                //   __m256 vec_v = vec_dy2dz2 + vec_dx2;
                //   if (vec_v[0]<cutoff2 && vec_v[1]<cutoff2 && vec_v[2]<cutoff2 && vec_v[3]<cutoff2 && 
                //       vec_v[4]<cutoff2 && vec_v[5]<cutoff2 && vec_v[6]<cutoff2 && vec_v[7]<cutoff2) 
                //   {
                //     __m256 vec_idx = vec_nx + (float)idx0;
                //     __m256 vec_val = beta * _mm256_sqrt_ps(1.0 - (vec_v * _1overCutoff2));
                //     __m256 vec_z = vec_val * vec_val;
                //     __m256 vec_num = (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z * (vec_z *
                //                                                                                  (vec_z *
                ...
                //                                0.216415572361227e8f) + 0.356644482244025e9f) +
                //                      0.144048298227235e10f);
                //     __m256 vec_den = (vec_z * (vec_z * (vec_z - 0.307646912682801e4f) + 0.347626332405882e7f) -
                //                      0.144048298227235e10f);
                //     __m256 vec_rValue = (0 - vec_num) / vec_den;
                //     __m256 vec_w = vec_rValue * pt.sdc;
                //     __m256 vec_w_real_product = vec_w * pt.real;
                //     __m256 vec_w_imag_product = vec_w * pt.imag;
